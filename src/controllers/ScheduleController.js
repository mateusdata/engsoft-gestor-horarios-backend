...
class ScheduleController {
  async createSchedules(req, res) {
    try {
      ...
      await sequelize.query(
        `INSERT INTO public.semestre${parseInt(
          semestre
        )}(dia, professor, disciplina, horario, semestre) VALUES (?, ?, ?, ?, ?)`,
        {
          replacements: [dia, professor, disciplina, horario, semestre],
      ...
      res
        .status(500)
        .json({
          message: "Houve um erro ao criar os dados.",
  ...
  async searchSchedules(req, res) {
    try {
      ...
      const dataFromDB = await sequelize.query(
        `SELECT * FROM public.semestre${parseInt(semestre)}`,
        { replacements: [semestre], type: sequelize.QueryTypes.SELECT }
      );
  ...
  async showAllSchedules(req, res) {
    try {
      ...
      // Após o loop, use flatMap para nivelar o array
      const flattenedData = allData.flatMap((data) => data);
  ...
  async currentHourlyData(req, res) {
    ...
    try {
      const query = await sequelize.query(
        `select prim_hor, prim_hor_materia, segu_hor, segu_hor_materia, terc_hor, terc_hor_materia, quar_hor, quar_hor_materia, quin_hor, quin_hor_materia, sext_hor, sext_hor_materia from bacharelado_si where id='${semestre}'`
      );
      res.send(query[0]);
    } catch {
      res.send("Erro.");
  ...
  async updateSchedules(req, res) {
    ...
    try {
      const query = await sequelize.query(
        `UPDATE bacharelado_si SET prim_hor='${prim_hor}', prim_hor_materia='${prim_hor_materia}', segu_hor='${segu_hor}', segu_hor_materia='${segu_hor_materia}', terc_hor='${terc_hor}', terc_hor_materia='${terc_hor_materia}', quar_hor='${quar_hor}', quar_hor_materia='${quar_hor_materia}', quin_hor='${quin_hor}', quin_hor_materia='${quin_hor_materia}', sext_hor='${sext_hor}', sext_hor_materia='${sext_hor_materia}' where id='${semestre}'`
      );
